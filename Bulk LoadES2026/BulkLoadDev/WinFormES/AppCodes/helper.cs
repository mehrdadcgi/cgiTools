...
namespace WinFormES
{
    public static class helper
    {
        ...
        try
        {
            if (string.IsNullOrWhiteSpace(strIn) || strIn.Trim().Length == 0)
                return string.Empty;

            // Keep ALL existing behavior (remove these characters)
            strIn = Regex.Replace(strIn, @"'", "").Trim();
            strIn = Regex.Replace(strIn, @"\r\n", "").Trim(); // keep your original step
            strIn = Regex.Replace(strIn, @"\n", "").Trim();
            ...
            // NEW: collapse multiple spaces to a single space (between names)
            strIn = Regex.Replace(strIn, @"\s{2,}", " ").Trim();
    ...
    public static String CleanInputfffss(string strIn)
        {
            try
            {
        ...
        public static string CleanStringFast(string input)
        {
