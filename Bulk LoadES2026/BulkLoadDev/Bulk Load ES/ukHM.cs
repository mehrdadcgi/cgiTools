            ...
            if (reader.HasRows)
            {
                ...
                while (reader.Read())
                {
                    ...
                    .Document(new Person
                    {
                        ...
                        LastName = helper.CleanInput(reader[""].ToString()),
                        midName = helper.CleanInput(""),
                        AltName = helper.CleanInput(""),

                        OrgName = helper.CleanInput(reader[""].ToString()),
                        Comment = helper.CleanInput(reader["Note"].ToString()),
                        ListSource = helper.CleanInput("EU_UNION"),
                        ListSourceID = helper.CleanInput("10"),
                        Location = helper.CleanInput(reader["country"].ToString()),
                        Address = helper.CleanInput(""),
                        City = helper.CleanInput(""),
                        Country = helper.CleanInput(reader["country"].ToString()+", "+ reader["Nationality"].ToString()),
