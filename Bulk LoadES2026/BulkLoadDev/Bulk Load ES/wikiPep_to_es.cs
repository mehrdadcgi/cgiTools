...
namespace ConsoleAppES
{
    public static class wikiPep_to_es
    {
        ...
        public static void addPepToESbyCountry(bool isLoadall)
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                if (isLoadall)
                {
                   ...
                   // helper.OutMsg("Add all sanc " + countryParam);
                }
               
                    BulkDescriptor list = GetAppendlist(countryParam);
                    AppendToIndex(list); 
                    helper.OutMsg("Appended " + countryParam);
        ...
        public static BulkDescriptor GetAppendlist(string countryParam)
        {
                ...
                if (reader.HasRows)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        .Document(new Person
                        {
                            ...
                            ListSourceID = helper.CleanInput("14"),
            ...
            helper.OutMsg("total record appended: "+i.ToString());
            return descriptor;
        }

        public static List<ExtSanctionData> addSancToEs(string countryParam)
        {
            helper.OutMsg("start read from db "+ countryParam + DateTime.Now.ToString());
            ...
            using (SqlCommand cmd = conn.CreateCommand())
            {
                ...
                if (reader.HasRows)
                {
                    ...
                    helper.OutMsg("bulk load completed ");
